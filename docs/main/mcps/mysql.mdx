...
1. Open a terminal (Windows: PowerShell or Command Prompt; macOS: Terminal).  
...
1. Get started by opening up the Claude menu on your computer and select “Settings…” Please note that these are not the Claude Account Settings found in the app window itself.
    ...
    ![claude desktop settings on MacOS](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/quickstart-menu.png)

2. Click on “Developer” in the left-hand bar of the Settings pane, and then click on “Edit Config”:

    ![claude desktop edit config](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/quickstart-developer.png)
...
3. Open `claude_desktop_config.json` in a text editor and Replace the file contents with this:
  <Tabs>
          ...
          "mcp_server_mysql": {
            ...
            "env": {
              ...
              "ALLOW_DELETE_OPERATION": "false",
                ...
                "APPDATA": "C:\\Users\\adria\\AppData\\Roaming\\"
            }
          }
        }
      }
    ...
    Replace `/absolute/path/to/node`(MacOS) or `absolute\\path\\to\\node` (Windows) with the full path to your `node` binary (use `which node` for MacOS or use `where node` for Windows).
    
    To get the MySQL credentials, please contact you nearest programmer.
...
2. Open `claude_desktop_config.json` in a text editor and add the `mcpServers` entry under the top‑level JSON object, for example:  
   ```json
   {
     "mcpServers": {
       "mcp_server_mysql": {
         ...
         "args": [
           "/absolute/path/to/mcp-server-mysql/dist/index.js"
         ],
         "env": {
   ...
   ```  
   - Replace `/absolute/path/to/node` with the full path to your `node` binary (use `which node`).  
   - Replace `/absolute/path/to/mcp-server-mysql` with your clone’s full path. citeturn2view0  

3. Save and close the file. -->

<!-- ### 3.3 Start and Test the Server

1. In your terminal, run the server directly to verify connectivity:  
   ```bash
   node dist/index.js
   ```  
2. Check the console for a successful MySQL connection message.  
3. In Claude Desktop, open any chat and run a SQL query tool, e.g.:  
   ```json
   {
     "tool": "mysql_query",
     "sql": "SHOW TABLES;"
   ...
   ```  
   If you see your database tables listed, the integration is working. -->
